"""Rerunnable migration: clean weekly focus mode and add dedicated Claude/CLI and MCP weeks."""
...
backup = DB_PATH.with_name(f"{DB_PATH.stem}_backup_before_v17_{stamp}{DB_PATH.suffix}")
...
with sqlite3.connect(DB_PATH) as conn:
    ...
    for week_number in range(1, 17):
        ...
        if week_number < 16:
            category = "Hands-On"
            key = f"v17:w{week_number:02d}:build"
            max_score = 100
            if week_number == 1:
                ...
                deliverable = "Claude instructions, bounded task, reviewed AI diff, CLI tests, Code Analyzer, deployment validation, and pull-request evidence."
            elif week_number == 15:
                ...
                deliverable = "MCP architecture, redacted configuration, tool inventory, read-only and controlled-action evidence, audit logs, skill file, and pull request."
            else:
            ...
            deliverable = "A production-style application, source repository, architecture documentation, tests, AI governance evidence, agent guardrails, release validation, and final classroom demonstration."

        conn.execute(
            """
            UPDATE assignments
            SET assignment_key=?, program_version='v17', title=?, instructions=?, deliverable=?, max_score=?
        ...
        conn.execute(
            """
            UPDATE assignments
            SET assignment_key=?, program_version='v17', title=?, instructions=?, deliverable=?
...
with sqlite3.connect(DB_PATH) as conn:
    ...
    print("Week 14: Claude AI and Salesforce CLI Development Workflow")
    print("Week 15: MCP Servers, Secure Tool Use, and Agent Skills")
    print("Week 16: Agentforce, AI Governance, and Final Production Demonstration")
